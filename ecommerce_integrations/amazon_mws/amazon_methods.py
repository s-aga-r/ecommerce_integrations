...
def get_products_details():
	...
	for marketplace in market_place_list:
		report_id = request_and_fetch_report_id(
			"_GET_FLAT_FILE_OPEN_LISTINGS_DATA_", None, None, market_place_list
		)
		...
		if report_id:
			...
			for asin_list in asin_chunked_list:
				products_response = call_mws_method(
					products.get_matching_product, marketplaceid=marketplace, asins=asin_list
				)
...
def request_and_fetch_report_id(report_type, start_date=None, end_date=None, marketplaceids=None):
	reports = get_reports_instance()
	report_response = reports.request_report(
		report_type=report_type, start_date=start_date, end_date=end_date, marketplaceids=marketplaceids
	)
	...
	for x in range(1, 10):
		...
		report_status = report_request_list_response.parsed["ReportRequestInfo"][
		...
		if report_status == "_SUBMITTED_" or report_status == "_IN_PROGRESS_":
			...
			generated_report_id = report_request_list_response.parsed["ReportRequestInfo"][
				"GeneratedReportId"
			]["value"]
			break
...
def create_item_price(amazon_item_json, item_code):
	...
	item_price.price_list = frappe.db.get_value(SETTINGS_DOCTYPE, SETTINGS_DOCTYPE, "price_list")
	if not ("ListPrice" in amazon_item_json.Product.AttributeSets.ItemAttributes):
		...
		item_price.price_list_rate = (
			amazon_item_json.Product.AttributeSets.ItemAttributes.ListPrice.Amount
		)
...
def get_orders(after_date):
	try:
		...
		while True:
			...
			if "NextToken" not in orders_response.parsed:
				break
...
def create_sales_order(order_json, after_date):
	...
	taxes_and_charges = frappe.db.get_value(SETTINGS_DOCTYPE, SETTINGS_DOCTYPE, "taxes_charges")
		...
		so = frappe.get_doc(
			{
				...
				"items": items,
				"company": frappe.db.get_value(SETTINGS_DOCTYPE, SETTINGS_DOCTYPE, "company"),
			}
		)
...
def create_customer(order_json):
	...
	if existing_customer_name:
		...
		mws_customer_settings = frappe.get_doc(SETTINGS_DOCTYPE)
...
def get_order_items(market_place_order_id):
	...
	order_items_response = call_mws_method(
		mws_orders.list_order_items, amazon_order_id=market_place_order_id
	)
	final_order_items = []

	order_items_list = return_as_list(order_items_response.parsed.OrderItems.OrderItem)

	warehouse = frappe.db.get_value(SETTINGS_DOCTYPE, SETTINGS_DOCTYPE, "warehouse")

	while True:
		for order_item in order_items_list:

			if "ItemPrice" not in order_item:
				price = 0
			else:
		...
		if "NextToken" not in order_items_response.parsed:
			break
